...
#pragma omp parallel for schedule(dynamic)
		REP(i, target_size.y) REP(j, target_size.x) {
			...
			REP(imgid, images.size())  {
				...
				if (!img_cur.meta.range.contain(i, j)) continue;
				if (!img_cur.valid_on_target(j, i)) continue;
			...
			if (!target_mask.get(i, j)) {		// first time to visit *p. Note that *p could be negative after visit.
				isum.write_to(p);
				target_mask.set(i, j);
			} else {
